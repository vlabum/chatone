...
public class ServerUnicastHandler {
    ...
    public void unicast(@ObservesAsync final ServerUnicastEvent event){
        ...
        @NotNull PacketUnicast packetUnicast = objectMapper.readValue(event.getMessage(), PacketUnicast.class);
        if (packetUnicast == null) return;
        @Nullable final String loginDst = packetUnicast.getLogin();
